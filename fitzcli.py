...
from fitz import ( # Corrected import statement
    TEXT_INHIBIT_SPACES,
...
def open_file(filename, password, show=False, pdf=True):
    ...
    doc = fitz.open("/content/20240716890312078.pdf")
...
# In fitzcli.py, replace the original gettext function with this one.
...
def gettext(args):
    ...
    # Otherwise, prepare to write to standard output.
    if args.output:
        textout = open(args.output, "wb")
    else:
        # Use the standard output buffer for binary writing
        textout = sys.stdout.buffer
    # --- MODIFICATION END ---
...
def main(args=None):
    ...
    # -------------------------------------------------------------------------
    if args is None:
        args = parser.parse_args([])  # create parameter arguments class with empty list
    else:
      args = parser.parse_args(args=[], namespace=args) # parse arguments into the provided namespace
