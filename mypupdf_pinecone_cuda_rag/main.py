# main.py (Version 8.4.0 - Auto-Adjusting Cores, OpenAI GPT-4.1)
#
...
# Uses OpenAI GPT-4.1 as the language model (instead of Google Gemini).
...
import openai   # <--- CHANGED: use OpenAI's library
...
app = FastAPI(
    title="LIT RAG with OpenAI GPT-4.1, PyMuPDF, and Cohere Reranker (Auto-Scaling)",
...
async def startup_event():
    ...
    # --- CHANGED: Use OpenAI API Key for GPT-4 ---
    openai_api_key = os.getenv("OPENAI_API_KEY")
    if not openai_api_key:
        raise ValueError("OPENAI_API_KEY environment variable not found.")
    openai.api_key = openai_api_key
    models["generation_model"] = "gpt-4o"  # or "gpt-4-1106-preview" or latest GPT-4, adjust as needed
...
async def process_single_query(query: str, namespace: str, max_retries: int = 3) -> str:
    ...
    for attempt in range(max_retries):
        try:
            ...
            prompt = (
                "You are an AI assistant explaining policy documents. Use only the provided CONTEXT; "
                "do not draw on outside knowledge. Be factual, clear and supportive. Format your ANSWER neatly—use bullet points when necessary. "
            ...
            # --- CHANGED: Use OpenAI API for completion ---
